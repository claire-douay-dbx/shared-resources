# Enterprise Data Engineering Standards - SDP SQL
...
| Audit columns | Add `current_timestamp() AS audit_timestamp` and `'source_name' AS source_system` as LAST columns |
...
| Audit columns | Add `current_timestamp() AS audit_timestamp` and `'silver_transformation' AS source_system` as LAST columns |
...
| Audit columns | Add `current_timestamp() AS audit_timestamp` and `'gold_aggregation' AS source_system` as LAST columns |
...
|-------------|-----------|-------------|----------|
| `audit_timestamp` | TIMESTAMP | Processing timestamp | Yes |
...
FROM catalog.schema.retail_articles_dataset
...
|---------|------|--------|---------|
| 1.0 | 2026-02 | Databricks Field Engineering | Generic enterprise standards template |
